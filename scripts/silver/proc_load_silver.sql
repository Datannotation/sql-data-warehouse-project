...
Script Purpose:
    ...
    -  Uses the 'INSERT' command to load data from bronze tables to silver tables involving various data transformations.
...
BEGIN
	...
	BEGIN TRY -- TRY CATCH block is used to catch any error or exception that may occur during ETL script execution
		...
		SELECT 
			...
			ELSE cid
			END AS cid,
			CASE WHEN bdate<'1926-01-01' OR bdate>'2011-01-01' THEN NULL
			ELSE bdate
			END AS bdate,
